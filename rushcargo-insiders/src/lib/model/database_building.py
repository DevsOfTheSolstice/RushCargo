...
from .classes import Building, Warehouse, WarehouseConnection, Branch
...
class WarehouseConnectionTable(BasicTable):
    # Constructor
    def __init__(self, database: Database):
        # Initialize Basic Table Class
        super().__init__(WAREHOUSE_CONN_TABLENAME, WAREHOUSE_CONN_ID, database)
    ...
    def __print(self) -> None:
        ...
        # Initialize Rich Table
        table = getTable("Warehouse Connection", nItems)

        # Add Table Columns
        table.add_column("ID", justify="left", max_width=ID_NCHAR)
        table.add_column("Warehouse From", justify="left", max_width=ID_NCHAR)
        table.add_column("Warehouse To", justify="left", max_width=ID_NCHAR)
        table.add_column("Distance", justify="left", max_width=DISTANCE_NCHAR)

        # Loop Over Items
        for item in self._items:
            # Intialize Warehouse Connection from Item Fetched
            c = WarehouseConnection.fromItemFetched(item)

            # Add Row to Rich Table
            table.add_row(
                str(c.warehouseConnId),
                str(c.warehouseFromId),
                str(c.warehouseToId),
                str(c.routeDistance),
            )
    ...
    # Returns Warehouse Connection Insert Query
    def __insertQuery(self):
    ...
    # Insert Warehouse Connection to Table
    def add(self, c: WarehouseConnection) -> None:
        # Get Query
        ...
        try:
            self._c.execute(
                query, [c.warehouseFromId, c.warehouseToId, c.routeDistance]
            )
            console.print(
                insertedRow(
                    f"Connection from ID {c.warehouseFromId} to ID {c.warehouseToId}",
                    self._tableName,
                ),
                style="success",
            )
    ...
    # Filter Items from Warehouse Connection Table
    def get(self, field: str, value, printItems: bool = True) -> bool:
    ...
    # Filter Items with Multiple Conditions from Warehouse Connection Table
    ...
    # Find Warehouse Connection ID from Warehouse Connection Table with the Warehouses From and To ID
    def findMult(
        self, warehouseFromId: int, warehouseToId: str
    ) -> WarehouseConnection | None:
        """
        Returns Warehouse Connection Object if it was Found. Otherwise, False
        """

        # Get Warehouse Connection
        if not self.getMult(
            [WAREHOUSE_CONN_WAREHOUSE_FROM_ID, WAREHOUSE_CONN_WAREHOUSE_TO_ID],
            [warehouseFromId, warehouseToId],
            False,
        ):
            return None

        # Get Warehouse Connection Object from Item Fetched
        return WarehouseConnection.fromItemFetched(self._items[0])

    # Find Warehouse Connection from Warehouse Connection Table with the Warehouse Connection ID
    def find(self, warehouseConnId: int) -> WarehouseConnection | None:
        """
        Returns Warehouse Connection Object if it was Found. Otherwise, False
        """

        # Get Warehouse Connection
        if not self.get(self._tablePKName, warehouseConnId, False):
            return None

        # Get Warehouse Connection Object from Item Fetched
        return WarehouseConnection.fromItemFetched(self._items[0])

    # Get All Items from Warehouse Connection Table
    def all(self, orderBy: str, desc: bool) -> None:
    ...
    # Modify Row from Warehouse Connection Table
    def modify(self, warehouseConnId: int, field: str, value) -> None:
        BasicTable._modify(self, warehouseConnId, field, value)

    # Remove Row from Warehouse Connection Table
    def remove(self, warehouseConnId: int) -> None:
        BasicTable._remove(self, warehouseConnId)
